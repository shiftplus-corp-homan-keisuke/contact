...
import { CreateInquiryRequest, UpdateInquiryRequest, InquiryPriority, InquiryStatus, SearchCriteria } from '../types/inquiry.types';
...
export class CreateInquiryDto implements CreateInquiryRequest {
  ...
  @IsOptional()
  @IsEnum(InquiryPriority, { message: '優先度は有効な値を選択してください' })
  priority?: InquiryPriority;
}

export class UpdateInquiryDto implements UpdateInquiryRequest {
  @IsOptional()
  @IsString({ message: 'タイトルは文字列で入力してください' })
...
export function validateInquiryPriority(priority: string): { isValid: boolean; errors: string[] } {
  ...
  if (priority && !validPriorities.includes(priority as InquiryPriority)) {
